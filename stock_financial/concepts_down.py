...
class ConceptDown(object):
    ...
    async def __get_content(self, link):  # 传入的是资源链接
        header = {
    ...
    async def __download_img(self, page):
        content = await self.__get_content(get_concept_url(page=page, psize=self._page_size))
        columns = {
    ...
    def run(self):
        ...
        tasks = [
            loop.create_task(
                self.__download_img(page)) for page in self._data]
